...
class test_new_account(TestCase):
    ...
    def test_can_create_new_account(self):
        ...
        self.assertEqual(AccountHub.objects.count(), 1)
        self.assertEqual(AccountStaticSatellite.objects.count(), 1)
        account_hub = AccountHub.objects.first()
        account_static_satellite = AccountStaticSatellite.objects.first()
