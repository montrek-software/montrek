...
from link_tables.tests.factories.link_tables_factories import AccountTransactionCategoryMapLinkFactory
...
class TestTransactionCategoryModelQueries(TestCase):
    ...
    def test_set_transaction_category_by_map(self):
        ...
        AccountTransactionCategoryMapLinkFactory(from_hub=account,
                                                 to_hub=transaction_cat_map.hub_entity)
        transaction1 = TransactionSatelliteFactory(
            transaction_party='SuperParty',
        )
        AccountTransactionLinkFactory(from_hub=account,
                                      to_hub=transaction1.hub_entity)

        transaction2 = TransactionSatelliteFactory()
        AccountTransactionLinkFactory(from_hub=account,
                                      to_hub=transaction2.hub_entity)

        transaction3 = TransactionSatelliteFactory()
        AccountTransactionLinkFactory(from_hub=account,
                                      to_hub=transaction3.hub_entity)
